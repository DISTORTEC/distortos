 ...
 * tryPopUntil()) to/from FifoQueue, both from thread and from interrupt context - these operations must return expected
 * result, cause expected number of context switches, finish within expected time frame and execute expected actions on
 * transferred object (various constructor types, destructor, swap, ...).
 */

class FifoQueueOperationsTestCase : public TestCase
{
private:
