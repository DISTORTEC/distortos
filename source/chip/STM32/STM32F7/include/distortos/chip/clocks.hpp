...
#ifdef DISTORTOS_CHIP_STM32F7_STANDARD_CLOCK_CONFIGURATION_ENABLE

#if defined(DISTORTOS_CHIP_STM32F7_PWR_OVER_DRIVE_ENABLE)

static_assert(DISTORTOS_CHIP_STM32F7_PWR_VOLTAGE_SCALE_MODE == 1 || DISTORTOS_CHIP_STM32F7_PWR_VOLTAGE_SCALE_MODE == 2,
		"Over-drive mode requires voltage scale 1 mode or voltage scale 2 mode!");
static_assert(DISTORTOS_CHIP_STM32F7_VDD_MV >= 2100,
...
#else	// !defined(DISTORTOS_CHIP_STM32F7_PWR_OVER_DRIVE_ENABLE)

/// over-drive index for \a maxPllOutFrequencies array (maxPllOutFrequencies[voltageScaleIndex][overDriveIndex])
constexpr uint8_t overDriveIndex {0};

#endif	// !defined(DISTORTOS_CHIP_STM32F7_PWR_OVER_DRIVE_ENABLE)

/// voltage scale index for \a maxPllOutFrequencies array (maxPllOutFrequencies[voltageScaleIndex][overDriveIndex])
constexpr uint8_t voltageScaleIndex {DISTORTOS_CHIP_STM32F7_PWR_VOLTAGE_SCALE_MODE - 1};
...
#ifdef DISTORTOS_CHIP_STM32F7_RCC_PLLS_ENABLE

/// input frequency of main and audio PLLs, Hz
#if defined(DISTORTOS_CHIP_STM32F7_RCC_PLLSRC_HSI)
constexpr uint32_t pllInFrequency {hsiFrequency};
#elif defined(DISTORTOS_CHIP_STM32F7_RCC_PLLSRC_HSE)
constexpr uint32_t pllInFrequency {DISTORTOS_CHIP_STM32F7_RCC_HSE_FREQUENCY};
#endif

/// VCO input frequency of main and audio PLLs, Hz
constexpr uint32_t vcoInFrequency {pllInFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLM};

static_assert(minVcoInFrequency <= vcoInFrequency && vcoInFrequency <= maxVcoInFrequency,
		"Invalid VCO input frequency!");

#ifdef DISTORTOS_CHIP_STM32F7_RCC_PLL_ENABLE

/// VCO output frequency of main PLL, Hz
constexpr uint32_t vcoOutFrequency {vcoInFrequency * DISTORTOS_CHIP_STM32F7_RCC_PLLN};
...
constexpr uint32_t pllOutFrequency {vcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLP};

static_assert(pllOutFrequency <= maxPllOutFrequency, "Invalid PLL output frequency!");

/// main PLL "Q" output frequency, Hz
constexpr uint32_t pllqOutFrequency {vcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLQ};
...
constexpr uint32_t pllrOutFrequency {vcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLR};

#endif	// defined(DISTORTOS_CHIP_STM32F76) || defined(DISTORTOS_CHIP_STM32F77)

#endif	// def DISTORTOS_CHIP_STM32F7_RCC_PLL_ENABLE

#ifdef DISTORTOS_CHIP_STM32F7_RCC_PLLI2S_ENABLE

/// VCO output frequency of PLLI2S, Hz
constexpr uint32_t plli2sVcoOutFrequency {vcoInFrequency * DISTORTOS_CHIP_STM32F7_RCC_PLLI2SN};
...
#ifdef DISTORTOS_CHIP_STM32F7_RCC_PLLI2SP

/// PLLI2S output frequency, Hz
constexpr uint32_t plli2sOutFrequency {plli2sVcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLI2SP};

static_assert(plli2sOutFrequency <= maxPlli2sOutFrequency, "Invalid PLLI2S output frequency!");

#endif	// def DISTORTOS_CHIP_STM32F7_RCC_PLLI2SP

/// PLLI2S "Q" output frequency, Hz
constexpr uint32_t plli2sqOutFrequency {plli2sVcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLI2SQ};

static_assert(plli2sqOutFrequency <= maxPlli2sqOutFrequency, "Invalid PLLI2S \"/Q\" output frequency!");

/// PLLI2S "R" output frequency, Hz
constexpr uint32_t plli2srOutFrequency {plli2sVcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLI2SR};

static_assert(plli2srOutFrequency <= maxPlli2srOutFrequency, "Invalid PLLI2S \"/R\" output frequency!");

#endif	// def DISTORTOS_CHIP_STM32F7_RCC_PLLI2S_ENABLE

#ifdef DISTORTOS_CHIP_STM32F7_RCC_PLLSAI_ENABLE

/// VCO output frequency of PLLSAI, Hz
constexpr uint32_t pllsaiVcoOutFrequency {vcoInFrequency * DISTORTOS_CHIP_STM32F7_RCC_PLLSAIN};
...
constexpr uint32_t pllsaiOutFrequency {pllsaiVcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLSAIP};

static_assert(pllsaiOutFrequency <= maxPllsaiOutFrequency, "Invalid PLLSAI output frequency!");

/// PLLSAI "Q" output frequency, Hz
constexpr uint32_t pllsaiqOutFrequency {pllsaiVcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLSAIQ};

static_assert(pllsaiqOutFrequency <= maxPllsaiqOutFrequency, "Invalid PLLSAI \"/Q\" output frequency!");

#ifdef DISTORTOS_CHIP_STM32F7_RCC_PLLSAIR

/// PLLSAI "R" output frequency, Hz
constexpr uint32_t pllsairOutFrequency {pllsaiVcoOutFrequency / DISTORTOS_CHIP_STM32F7_RCC_PLLSAIR};

static_assert(pllsairOutFrequency <= maxPllsairOutFrequency, "Invalid PLLSAI \"/R\" output frequency!");

#endif	// def DISTORTOS_CHIP_STM32F7_RCC_PLLSAIR

#endif	// def DISTORTOS_CHIP_STM32F7_RCC_PLLSAI_ENABLE

/// PLL48CLK frequency, Hz
#if defined(DISTORTOS_CHIP_STM32F7_RCC_PLL48CLK_PLLQ)
constexpr uint32_t pll48clkFrequency {pllqOutFrequency};
#elif defined(DISTORTOS_CHIP_STM32F7_RCC_PLL48CLK_PLLSAIP)
constexpr uint32_t pll48clkFrequency {pllsaiOutFrequency};
#endif	// defined(DISTORTOS_CHIP_STM32F7_RCC_PLL48CLK_PLLSAIP)

#endif	// def DISTORTOS_CHIP_STM32F7_RCC_PLLS_ENABLE

/// SYSCLK frequency, Hz
#if defined(DISTORTOS_CHIP_STM32F7_RCC_SYSCLK_HSI)
constexpr uint32_t sysclkFrequency {hsiFrequency};
#elif defined(DISTORTOS_CHIP_STM32F7_RCC_SYSCLK_HSE)
constexpr uint32_t sysclkFrequency {DISTORTOS_CHIP_STM32F7_RCC_HSE_FREQUENCY};
#elif defined(DISTORTOS_CHIP_STM32F7_RCC_SYSCLK_PLL)
constexpr uint32_t sysclkFrequency {pllOutFrequency};
#endif	// defined(DISTORTOS_CHIP_STM32F7_RCC_SYSCLK_PLL)

#if defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_PLL48CLK) || defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_SYSCLK)

/// SDMMC1 adapter clock frequency, Hz
#if defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_PLL48CLK)
constexpr uint32_t sdmmc1clkFrequency {pll48clkFrequency};
#elif defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_SYSCLK)
constexpr uint32_t sdmmc1clkFrequency {sysclkFrequency};
#endif	// defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_SYSCLK)

static_assert(sdmmc1clkFrequency <= maxSdmmcFrequency, "Invalid SDMMC1 adapter clock frequency!");

#endif	// defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_PLL48CLK) ||
		// defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_SYSCLK)

#if defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_PLL48CLK) || defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_SYSCLK)

/// SDMMC2 adapter clock frequency, Hz
#if defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_PLL48CLK)
constexpr uint32_t sdmmc2clkFrequency {pll48clkFrequency};
#elif defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_SYSCLK)
constexpr uint32_t sdmmc2clkFrequency {sysclkFrequency};
#endif	// defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_SYSCLK)

static_assert(sdmmc2clkFrequency <= maxSdmmcFrequency, "Invalid SDMMC2 adapter clock frequency!");

#endif	// defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_PLL48CLK) ||
		// defined(DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_SYSCLK)

#else	// !def DISTORTOS_CHIP_STM32F7_STANDARD_CLOCK_CONFIGURATION_ENABLE

/// SYSCLK frequency, Hz
constexpr uint32_t sysclkFrequency {DISTORTOS_CHIP_STM32F7_RCC_SYSCLK_FREQUENCY};

#if DISTORTOS_CHIP_STM32F7_RCC_PLL48CLK_FREQUENCY != 0

/// PLL48CLK frequency, Hz
constexpr uint32_t pll48clkFrequency {DISTORTOS_CHIP_STM32F7_RCC_PLL48CLK_FREQUENCY};

#endif	// DISTORTOS_CHIP_STM32F7_RCC_PLL48CLK_FREQUENCY != 0

#if DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_FREQUENCY != 0

/// SDMMC1 adapter clock frequency, Hz
constexpr uint32_t sdmmc1clkFrequency {DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_FREQUENCY};

#endif	// DISTORTOS_CHIP_STM32F7_RCC_SDMMC1CLK_FREQUENCY != 0

#if DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_FREQUENCY != 0

/// SDMMC2 adapter clock frequency, Hz
constexpr uint32_t sdmmc2clkFrequency {DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_FREQUENCY};

#endif	// DISTORTOS_CHIP_STM32F7_RCC_SDMMC2CLK_FREQUENCY != 0
...
#endif	// !def DISTORTOS_CHIP_STM32F7_STANDARD_CLOCK_CONFIGURATION_ENABLE

/// AHB frequency, Hz
constexpr uint32_t ahbFrequency {sysclkFrequency / DISTORTOS_CHIP_STM32F7_RCC_HPRE};

/// APB1 frequency, Hz
constexpr uint32_t apb1Frequency {ahbFrequency / DISTORTOS_CHIP_STM32F7_RCC_PPRE1};

static_assert(apb1Frequency <= maxApb1Frequency, "Invalid APB1 (low speed) frequency!");

/// APB2 frequency, Hz
constexpr uint32_t apb2Frequency {ahbFrequency / DISTORTOS_CHIP_STM32F7_RCC_PPRE2};
