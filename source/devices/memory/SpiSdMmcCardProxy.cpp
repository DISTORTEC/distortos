/**
 * \file
 * \brief SpiSdMmcCardProxy class implementation
 *
...
SpiSdMmcCardProxy::SpiSdMmcCardProxy(SpiSdMmcCard& spiSdMmcCard) :
		spiDeviceProxy_{spiSdMmcCard.spiDevice_},
...
std::pair<int, size_t> SpiSdMmcCardProxy::program(const uint64_t address, const void* const buffer,
		const size_t size) const
{
	...
	if (buffer == nullptr || address % SpiSdMmcCard::blockSize != 0 || size % SpiSdMmcCard::blockSize != 0)
		return {EINVAL, {}};
	...
	for (size_t block {}; block < blocks; ++block)
	{
		const auto ret = writeDataBlock(spiMasterProxy, blocks == 1 ? startBlockToken : startBlockWriteToken,
				bufferUint8 + block * SpiSdMmcCard::blockSize, SpiSdMmcCard::blockSize, std::chrono::milliseconds{250});
...
std::pair<int, size_t> SpiSdMmcCardProxy::read(const uint64_t address, void* const buffer, const size_t size) const
{
	...
	for (size_t block {}; block < blocks; ++block)
	{
		const auto ret = readDataBlock(spiMasterProxy, bufferUint8 + block * SpiSdMmcCard::blockSize,
				SpiSdMmcCard::blockSize, std::chrono::milliseconds{100});
...
void SpiSdMmcCardProxy::deinitialize() const
{
	spiSdMmcCard_.blocksCount_ = {};
	spiSdMmcCard_.blockAddressing_ = {};
	spiSdMmcCard_.type_ = {};
}

int SpiSdMmcCardProxy::initialize() const
{
	CHECK_FUNCTION_CONTEXT();
	...
	}
	{
		...
		if (std::get<1>(ret) == r1InIdleStateMask)
		{
			...
			spiSdMmcCard_.type_ = SpiSdMmcCard::Type::sdVersion2;
		}
	}
	{
		...
		while (1)
		{
			const auto ret = executeAcmd41(spiMasterProxy, spiSdMmcCard_.type_ == SpiSdMmcCard::Type::sdVersion2);
			if (ret.first != 0)
				return ret.first;
			if (ret.second == 0)
			{
				if (spiSdMmcCard_.type_ == SpiSdMmcCard::Type::unknown)
					spiSdMmcCard_.type_ = SpiSdMmcCard::Type::sdVersion1;

				break;
			}
			if (ret.second != r1InIdleStateMask || TickClock::now() >= deadline)
			{
				if (spiSdMmcCard_.type_ == SpiSdMmcCard::Type::sdVersion2)
	...
	if (spiSdMmcCard_.type_ == SpiSdMmcCard::Type::unknown)
	{
		...
		while (1)
		{
			...
			if (ret.second == 0)
			{
				spiSdMmcCard_.type_ = SpiSdMmcCard::Type::mmc;
				break;
			}
			if (ret.second != r1InIdleStateMask || TickClock::now() >= deadline)
				return ret.second != r1InIdleStateMask ? EIO : ETIMEDOUT;
	...
	if (spiSdMmcCard_.type_ == SpiSdMmcCard::Type::sdVersion2)
	{
		...
		spiSdMmcCard_.blockAddressing_ = (std::get<2>(ret) & ocrCcsMask) != 0;
	}

	if (spiSdMmcCard_.blockAddressing_ == false)
	{
		const auto ret = executeCmd16(spiMasterProxy, SpiSdMmcCard::blockSize);
		if (ret.first != 0)
			return ret.first;
		if (ret.second != 0)
			return EIO;
...

	{
		...
		spiSdMmcCard_.blocksCount_ =
				(static_cast<uint64_t>(csd.csdV2.cSize) + 1) * 512 * 1024 / SpiSdMmcCard::blockSize;
	}
